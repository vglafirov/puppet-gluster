...
define gluster::brick(
	...
	# http://git.kernel.org/?p=linux/kernel/git/torvalds/linux-2.6.git;a=blob;f=Documentation/filesystems/xfs.txt;hb=HEAD
		...
		# XXX: this filesystem has not yet been optimized for performance

		# mkfs w/ uuid command
		# XXX: currently it's not possible to set the UUID for btrfs
XXX		$mkfs_exec = "${::gluster::params::program_mkfs_btrfs} -U '${valid_fsuuid}' ${dev2}"
